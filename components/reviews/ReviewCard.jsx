...
export default function ReviewCard({ review, variant = 'default' }) {
  if (variant === 'minimal') {
    ...
    return (
      <Wrapper 
        {...wrapperProps}
        className="relative flex flex-col h-full bg-white border border-black/[0.04] hover:border-black/[0.1] rounded-2xl p-6 md:p-8 transition-colors duration-300 block cursor-pointer" 
  ...
  return (
    <article className="relative flex flex-col h-full bg-white border border-black/[0.04] hover:border-black/[0.1] rounded-2xl p-5 pb-6 transition-colors duration-300" aria-label={`Reseña de ${review.authorName}`}>
