...
export default function ReviewCard({ review, variant = 'default' }) {
  return (
    <article className="relative flex flex-col h-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-5 pb-6 transition-all duration-200 hover:border-[#3a3a3a]" aria-label={`Reseña de ${review.authorName}`}>
      ...
      {review.text && <p className="text-sm text-[#ccc] leading-relaxed line-clamp-4 mb-4">{review.text}</p>}
      <div className="mt-auto flex items-center gap-1.5 pt-3 border-t border-[#2a2a2a]">
        ...
        <span className="text-xs text-[#666]">Reseña de Google</span>
      </div>
    </article>
  );
}