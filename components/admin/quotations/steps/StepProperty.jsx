...
export default function StepProperty({ selected, onChange, onNext }) {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-4">Seleccionar propiedad(es)</h2>
      <p className="text-sm text-[#666] mb-4">Elegí entre 1 y 3 propiedades para incluir en la propuesta.</p>

      <input
        type="text" placeholder="Buscar por nombre o ciudad..."
        value={search} onChange={(e) => setSearch(e.target.value)}
        className="w-full border border-[#ddd] rounded-lg px-4 py-2.5 text-sm outline-none focus:border-[var(--color-brand)] mb-4"
      />
          ...
          {filtered.map(prop => {
            ...
            return (
              <div key={prop._id}
                ...
                className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${isSelected ? 'border-[var(--color-brand)] bg-[var(--color-brand)]/5' : 'border-[#eee] hover:border-[#ddd]'}`}
              >
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${isSelected ? 'border-[var(--color-brand)] bg-[var(--color-brand)]' : 'border-[#ccc]'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#0F172A] truncate">{prop.name}</p>
                  <p className="text-xs text-[#999]">{prop.location?.city || ''} · {prop.type || ''}</p>
                </div>
