        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {AGENTS.map((agent, i) => (
            <ScrollReveal key={agent.id} delay={i * 80}>
              <div className="bg-white border border-[var(--color-border)] rounded-2xl p-6 flex flex-col items-center text-center shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-card-hover)] hover:-translate-y-0.5 transition-all duration-200">
                {/* Avatar */}
                ...
                {/* Specialty */}
                <p className="text-[13px] text-[var(--color-ink-tertiary)] mb-3">
                  {agent.specialty}
                </p>
                ...
                {/* Contact Button */}
                <button className="w-full h-10 border border-[var(--color-border)] hover:border-[var(--color-brand)] hover:text-[var(--color-brand)] rounded-lg text-[13px] font-medium text-[var(--color-ink-secondary)] transition-all duration-150 flex items-center justify-center gap-2">
