...
const FIELD_MASK = [
  ...
  'reviews(name,authorAttribution,rating,publishTime,relativePublishTimeDescription,text,originalText)',
].join(',');
...
export async function fetchPlaceReviews(placeId, languageCode = 'es') {
  ...
  if (!GOOGLE_API_KEY) {
    throw new Error('GOOGLE_API_KEY no configurada. Verificá las variables de entorno en Vercel → Settings → Environment Variables.');
  }
