...
const limit = pLimit(3); // upload up to 3 images concurrently
...
async function run() {
  ...
  for (const doc of allProps) {
    ...
    try {
      ...
      // 1. Delete the bad images (logos) from Cloudinary
      if (doc.images && doc.images.length > 0) {
        console.log(`Deleting ${doc.images.length} old images...`);
      ...
      // 2. Fetch the pristine legacy URL
